    ...
    save_promise.then( async function(path_chosen) {
        ...
        if(path_chosen.canceled == false) {
            ...
            if (!FS.existsSync(path_chosen.filePath)) {
                ...
                res = FS.openSync( PATH2.join(path_chosen.filePath,'TAGGING-MEMES.ndjson') , 'w');
                ...
                while( tagging_meme_record_tmp != undefined ) {
                    ...
                    content += "\n";
                    FS.appendFile( PATH2.join(path_chosen.filePath,'TAGGING-MEMES.ndjson') , content, (err) => {
                        if(err) console.log(err);
                    });
                    tagging_meme_record_tmp = await iter_tagging_meme()
                }
                //export collection records to json as well
                res = FS.openSync( PATH2.join(path_chosen.filePath,'COLLECTIONS.ndjson') , 'w');
                ...
                while( collection_record_tmp != undefined ) {
                    ...
                    content += "\n";
                    FS.appendFile( PATH2.join(path_chosen.filePath,'COLLECTIONS.ndjson') , content, (err) => {
                        if(err) console.log(err);
                    });
                ...
                res = FS.openSync( PATH2.join(path_chosen.filePath,'COLLECTIONS-IMAGES.ndjson') , 'w');
                ...
                while( collection_image_record_tmp != undefined ) {
                    ...
                    content += "\n";
                    FS.appendFile( PATH2.join(path_chosen.filePath,'COLLECTIONS-IMAGES.ndjson') , content, (err) => {
                        if(err) console.log(err);
                    });
                ...
                res = FS.openSync( PATH2.join(path_chosen.filePath,'COLLECTIONS-MEMES.ndjson') , 'w');
                ...
                while( collection_meme_record_tmp != undefined ) {
                    ...
                    content += "\n";
                    FS.appendFile( PATH2.join(path_chosen.filePath,'COLLECTIONS-MEMES.ndjson') , content, (err) => {
                        if(err) console.log(err);
                    });
