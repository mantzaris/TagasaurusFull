...
async function Collection_Profile_Image_Search_Action() {
  ...
  let profile_img_sorted = await SEARCH_MODULE.Collection_Profile_Image_Search_Fn(
    collection_profile_search_obj,
    current_collection_obj.collectionGalleryFiles,
...
async function Search_Collections_Search_Action() {
  ...
  search_collection_results = await SEARCH_MODULE.Collection_Search_DB(
    collection_search_obj,
    collection_db_iterator,
    Get_Tagging_Annotation_From_DB,
    MAX_COUNT_SEARCH_RESULTS
  );
    ...
    if (save_modal_meme_tagging.style.display == 'block') {
      if (ev.target.id == 'save-file-modal-view') {
        ...
        if (results.canceled == false) {
          ...
          FS.copyFileSync(
            PATH.join(TAGA_DATA_DIRECTORY, PATH.basename(document.getElementById('modal-image-clicked-displayimg-id').src)),
            output_name,
    ...
    if (save_profile_div.style.display == 'block') {
      if (ev.target.id == 'save-file-profile-img') {
        ...
        if (results.canceled == false) {
          const output_name = results.filePath;
          FS.copyFileSync(
            PATH.join(TAGA_DATA_DIRECTORY, PATH.basename(document.getElementById('collection-profile-image-img-id').src)),
            output_name,
            FS.constants.COPYFILE_EXCL
          );
          alert('saved file to download');
