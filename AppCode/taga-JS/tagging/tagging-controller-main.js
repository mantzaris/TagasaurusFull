...
const { CLOSE_ICON_RED, CLOSE_ICON_BLACK, HASHTAG_ICON } = require(PATH.join(
  __dirname,
  '..',
  'constants',
  'constants-icons.js'
)); //require(PATH.resolve()+PATH.sep+'constants'+PATH.sep+'constants-icons.js');
  ...
  if (super_res.length > 0) {
    for (let face_ii = 0; face_ii < super_res.length; face_ii++) {
      for (let [key, value] of Object.entries(super_res[face_ii].expressions)) {
        if (
          Object.keys(file_annotation_obj['taggingEmotions']).includes(key) ==
          false
        ) {
      ...
      if (ft_res == 'gif') {
        let { faceDescriptors, faceEmotions } =
          await Get_Image_Face_Expresssions_From_GIF(
            PATH.join(
              TAGA_DATA_DIRECTORY,
              current_image_annotation['fileName']
            ),
            true,
            true
          );
        ...
        super_res = await Get_Image_Face_Expresssions_From_File(
          PATH.join(TAGA_DATA_DIRECTORY, current_image_annotation['fileName'])
        );
        current_image_annotation['taggingEmotions'] = await Auto_Fill_Emotions(
          super_res,
...
document
  .getElementById(`auto-fill-emotions-check-box-id`)
...
async function Update_Tagging_MEME_Connections(
  ...
  return await DB_MODULE.Update_Tagging_MEME_Connections(
    fileName,
    current_image_memes,
    new_image_memes
  );
  ...
  if (ft_res == 'image') {
    ...
    btn_next.onclick = async () => {
      if (page_num < total_pages) {
        ...
        center_gallery_element.src = await GENERAL_HELPER_FNS.PDF_page_2_image(
          pdf,
          page_num
        );
      }
...
function Description_Hashtags_Display_Fill() {
  document.getElementById('description-textarea-id').value =
    current_image_annotation['taggingRawDescription'];
  ...
  document
    .getElementById('hashtags-innerbox-displayhashtags-id')
    .appendChild(list);
}
...
async function Emotion_Display_Fill() {
  ...
  emotion_keys.forEach(function (key_tmp) {
    document.getElementById(`emotion-delete-button-id-${key_tmp}`).onclick =
      function () {
  ...
  for (var key of emotion_keys) {
    ...
    document.getElementById('emotion-range-id-' + key).value =
      current_image_annotation['taggingEmotions'][key];
  }
}
...
async function Add_New_Emotion() {
  let new_emotion_text = document.getElementById(
    'emotions-new-emotion-textarea-id'
  ).value;
  ...
  if (new_emotion_text) {
    ...
    if (boolean_included == false) {
      current_image_annotation['taggingEmotions'][new_emotion_text] =
        new_emotion_value;
      await Update_Tagging_Annotation_DB(current_image_annotation);
    }
...
async function Meme_View_Fill() {
  ...
  for (ii = 0; ii < meme_choices.length; ii++) {
    if (
      FS.existsSync(`${TAGA_DATA_DIRECTORY}${PATH.sep}${meme_choices[ii]}`) ==
      true
    ) {
      document.getElementById(
        `meme-toggle-id-${meme_choices[ii]}`
      ).checked = true;
    }
  }
...
async function Meme_Image_Clicked(meme_file_name) {
  let modal_meme_click_top_id_element = document.getElementById(
    'modal-meme-clicked-top-id'
  );
  modal_meme_click_top_id_element.style.display = 'block';
  // Get the button that opens the modal
  let meme_modal_close_btn = document.getElementById(
    'modal-meme-clicked-close-button-id'
  );
  // When the user clicks on the button, close the modal
  meme_modal_close_btn.onclick = function () {
    let name_type = document.getElementById(
  ...
  let meme_click_modal_div = document.getElementById(
  ...
  let clicked_meme_element = document.getElementById(
    `memes-image-img-id-${meme_file_name}`
  );
  ...
  if (node_type == 'IMG') {
    ...
    pdf = await pdfjsLib.getDocument(
      `${TAGA_DATA_DIRECTORY}${PATH.sep}${meme_file_name}`
    ).promise;
  ...
  meme_click_modal_div.insertAdjacentHTML(
    'beforeend',
    meme_click_modal_body_html_tmp
  );
  let meme_image_annotations = await Get_Tagging_Annotation_From_DB(
    meme_file_name
  );
  ...
  document.getElementById(
    'modal-meme-clicked-emotion-list-div-container-id'
  ).innerHTML = modal_emotions_html_tmp;
  ...
  ).innerHTML = modal_tags_html_tmp;

  //event listeners for the pdf view
  if (node_type == 'DIV') {
    const center_gallery_element = document.getElementById(
      'modal-meme-clicked-displayimg-id'
    );
...
function Make_Blank_Tagging_View() {
  ...
  document.getElementById('hashtags-innerbox-displayhashtags-id').innerHTML =
    ''; //clear the display for the hashtags
}
...
async function New_Image_Display(n) {
  if (current_image_annotation == undefined || n == 0) {
    ...
    current_image_annotation = await Step_Get_Annotation(
      current_image_annotation.fileName,
      1
    );
  } else if (n == -1) {
...
async function First_Display_Init() {
  ...
  if (records_remaining == 0) {
    ...
    //console.log('tagging_name_param = ', tagging_name_param)
    current_image_annotation = await Get_Tagging_Annotation_From_DB(
      tagging_name_param
    );
    Load_State_Of_Image_IDB();
...
async function Save_Image_Annotation_Changes() {
  ...
  for (var ii = 0; ii < current_memes.length; ii++) {
    if (
      FS.existsSync(`${TAGA_DATA_DIRECTORY}${PATH.sep}${current_memes[ii]}`) ==
      true
    ) {
      let meme_boolean_tmp = document.getElementById(
        `meme-toggle-id-${current_memes[ii]}`
      ).checked;
  ...
  let processed_tag_word_list =
    DESCRIPTION_PROCESS_MODULE.process_description(rawDescription);
  ...
  for (var key of Object.keys(current_image_annotation['taggingEmotions'])) {
    current_image_annotation['taggingEmotions'][key] = document.getElementById(
      'emotion-range-id-' + key
    ).value;
  }
  await Update_Tagging_Annotation_DB(current_image_annotation);
  await Update_Tagging_MEME_Connections(
    current_image_annotation.fileName,
    current_memes,
...
async function Load_Default_Taga_Image() {
  ...
  if (
    FS.existsSync(`${TAGA_DATA_DIRECTORY}${PATH.sep}${'Taga.png'}`) == false
  ) {
    FS.copyFileSync(
      taga_source_path,
...
async function Delete_Image() {
  if (
    FS.existsSync(
      `${TAGA_DATA_DIRECTORY}${PATH.sep}${current_image_annotation.fileName}`
    ) == true
  ) {
    FS.unlinkSync(
      `${TAGA_DATA_DIRECTORY}${PATH.sep}${current_image_annotation.fileName}`
  ...
  await Update_Tagging_MEME_Connections(
    current_image_annotation.fileName,
    current_image_annotation.taggingMemeChoices,
    []
  );
  await Handle_Delete_Image_MEME_references(current_image_annotation.fileName);

  await Handle_Delete_Collection_IMAGE_references(
    current_image_annotation.fileName
  );
  await Handle_Delete_Collection_MEME_references(
    current_image_annotation.fileName
  );
...
async function Load_New_Image(filename) {
  ...
  if (!filename) {
    ...
    if (
      ...
      PATH.dirname(result.filePaths[0]) == TAGA_DATA_DIRECTORY
    ) {
      return;
    }
    last_user_image_directory_chosen = PATH.dirname(result.filePaths[0]);
    filenames = await MY_FILE_HELPER.Copy_Non_Taga_Files(
      result,
      TAGA_DATA_DIRECTORY,
      Get_Tagging_Hash_From_DB
    );
  } else {
  ...
  for (let filename of filenames) {
    ...
    let tagging_entry_tmp = JSON.parse(
      JSON.stringify(TAGGING_DEFAULT_EMPTY_IMAGE_ANNOTATION)
    ); //cloning obj
    ...
    if (hash_present == undefined) {
      ...
      if (ft_res.mime.includes('image') == true) {
        if (ft_res.ext == 'gif') {
          ...
          if (default_auto_fill_emotions == true) {
            let super_res =
              await Get_Image_Face_Descriptors_And_Expresssions_From_File(
                PATH.join(TAGA_DATA_DIRECTORY, tagging_entry_tmp['fileName'])
              );
            tagging_entry_tmp['taggingEmotions'] = await Auto_Fill_Emotions(
              super_res,
              tagging_entry_tmp
            );
            tagging_entry_tmp['faceDescriptors'] =
              await Get_Face_Descriptors_Arrays(super_res);
          } else {
            let super_res = await Get_Image_Face_Descriptors_From_File(
              PATH.join(TAGA_DATA_DIRECTORY, tagging_entry_tmp['fileName'])
            );
            tagging_entry_tmp['faceDescriptors'] =
              await Get_Face_Descriptors_Arrays(super_res);
        ...
        if (
          !(
            ft_res.mime.includes('mp4') ||
            ft_res.mime.includes('mkv') ||
            ft_res.mime.includes('mov')
          )
        ) {
          // 'ffmpegDecode'
          ...
          FS.unlink(
            ...
            (err) => {
              if (err)
                console.log('problem deleting video copied after ffmpeg', err);
            }
          );
        ...
        if (default_auto_fill_emotions == true) {
          let { video_face_descriptors, emotions_total } =
            await Get_Image_FaceApi_From_VIDEO(
              PATH.join(TAGA_DATA_DIRECTORY, tagging_entry_tmp['fileName']),
              true,
              false
            );
          ...
          let { video_face_descriptors } = await Get_Image_FaceApi_From_VIDEO(
            PATH.join(TAGA_DATA_DIRECTORY, tagging_entry_tmp['fileName']),
            false,
            false
          );
        ...
        if (
          !(
            ft_res.mime.includes('mp3') ||
            ft_res.mime.includes('wav') ||
            ft_res.mime.includes('mpeg')
          )
        ) {
...
//using the WEBCAM
document.getElementById('load-webcam-input-button-id').onclick =
  async function () {
    let modal_meme_click_top_id_element = document.getElementById(
      'modal-webcam-clicked-top-id'
    );
    modal_meme_click_top_id_element.style.display = 'block';
    document.getElementById('webcam-video-id').style.display = 'block';

    let meme_modal_close_btn = document.getElementById(
      'modal-webcam-clicked-close-button-id'
    );
    // When the user clicks on the button, close the modal
    meme_modal_close_btn.onclick = function () {
      Close_Modal();
    ...
    let select_capture_button = document.getElementById(
    ...
    let display_area_element = document.getElementById(
      'modal-webcam-clicked-image-gridbox-id'
    );
    ...
    select_capture_button.onclick = async function () {
      ...
      const final_path = PATH.join(
        await IPC_RENDERER.invoke('getDownloadsFolder'),
        outputname
      );
...
async function Search_Images() {
  ...
  // Get the button that opens the modal
  let meme_modal_close_btn = document.getElementById(
    'modal-search-close-exit-view-button-id'
  );
  // When the user clicks on the button, close the modal
  meme_modal_close_btn.onclick = function () {
    const search_res_children = document.getElementById(
      'modal-search-images-results-grid-div-area-id'
    ).children;
    const search_meme_res_children = document.getElementById(
      'modal-search-meme-images-results-grid-div-area-id'
    ).children;
  ...
  document.getElementById(
    'modal-search-emotion-label-value-textarea-entry-id'
  ).value = '';
  document.getElementById('modal-search-emotion-value-range-entry-id').value =
    '0';
  document.getElementById(
    'modal-search-emotion-label-value-display-container-div-id'
  ).innerHTML = '';
  document.getElementById(
    'modal-search-images-results-grid-div-area-id'
  ).innerHTML = '';
  ...
  document.getElementById('modal-search-main-reset-button-id').onclick =
    function () {
      ...
      document.getElementById(
        'modal-search-emotion-value-range-entry-id'
      ).value = '0';
      document.getElementById(
        'modal-search-emotion-label-value-display-container-div-id'
      ).innerHTML = '';
  ...
  document.getElementById('modal-search-emotion-entry-button-id').onclick =
    function () {
      let entered_emotion_label = document.getElementById(
        'modal-search-emotion-label-value-textarea-entry-id'
      ).value;
      let emotion_search_entry_value = document.getElementById(
        'modal-search-emotion-value-range-entry-id'
      ).value;
      if (entered_emotion_label != '') {
        tagging_search_obj['emotions'][entered_emotion_label] =
          emotion_search_entry_value;
      }
      document.getElementById(
        'modal-search-emotion-label-value-textarea-entry-id'
      ).value = '';
      ...
      let image_emotions_div_id = document.getElementById(
        'modal-search-emotion-label-value-display-container-div-id'
      );
      ...
      Object.keys(tagging_search_obj['emotions']).forEach((emotion_key) => {
        document
          .getElementById(
            `modal-search-emotion-remove-button-id-${emotion_key}`
          )
          .addEventListener('click', function () {
            let search_emotion_search_span_html_obj = document.getElementById(
              `modal-search-emotion-label-value-span-id-${emotion_key}`
            );
  ...
  //search meme results
  let search_meme_results_output = document.getElementById(
    'modal-search-meme-images-results-grid-div-area-id'
  );
  ...
  search_results.forEach((file) => {
    if (FS.existsSync(`${TAGA_DATA_DIRECTORY}${PATH.sep}${file}`) == true) {
      document.getElementById(
        `modal-image-search-result-single-image-img-id-${file}`
        ...
        Load_State_Of_Image_IDB();
        document.getElementById('search-modal-click-top-id').style.display =
          'none';
      };
    }
  });
  search_meme_results.forEach((file) => {
    if (FS.existsSync(`${TAGA_DATA_DIRECTORY}${PATH.sep}${file}`) == true) {
      document.getElementById(
        `modal-image-search-result-single-meme-image-img-id-${file}`
      ).onclick = async function () {
        const search_res_children = document.getElementById(
          'modal-search-images-results-grid-div-area-id'
        ).children;
  ...
  document.getElementById('modal-search-similar-button-id').onclick =
    function () {
...
async function Modal_Search_Entry(
  ...
  search_obj_similar_tmp = {}
) {
  search_obj_tmp = tagging_search_obj;
  if (search_similar == false) {
    //annotation tags
    let search_tags_input = document.getElementById(
      'modal-search-tag-textarea-entry-id'
    ).value;
    let split_search_string = search_tags_input.split(reg_exp_delims);
    let search_unique_search_terms = [...new Set(split_search_string)];
    search_unique_search_terms = search_unique_search_terms.filter(
      (tag) => tag !== ''
    );
    search_obj_tmp['searchTags'] = search_unique_search_terms;
    //meme tags now
    let search_meme_tags_input = document.getElementById(
      'modal-search-meme-tag-textarea-entry-id'
    ).value;
    let split_meme_search_string = search_meme_tags_input.split(reg_exp_delims);
    let search_unique_meme_search_terms = [
      ...new Set(split_meme_search_string),
    ];
    search_unique_meme_search_terms = search_unique_meme_search_terms.filter(
      (tag) => tag !== ''
    );
  ...
  search_results = await SEARCH_MODULE.Image_Search_DB(
    search_obj_tmp,
    tagging_db_iterator,
  ...
  search_meme_results = await SEARCH_MODULE.Image_Meme_Search_DB(
    search_obj_tmp,
  ...
  search_image_results_output.innerHTML = search_display_inner_tmp;
  //search meme results
  search_meme_results_output = document.getElementById(
    'modal-search-meme-images-results-grid-div-area-id'
  );
  search_meme_results_output.innerHTML = '';
  search_display_inner_tmp = '';
...
async function Modal_Search_Similar() {
  ...
  search_obj_similar_tmp.searchMemeTags =
    current_image_annotation.taggingMemeChoices;
  search_obj_similar_tmp.faceDescriptors =
    current_image_annotation.faceDescriptors;
...
async function Add_New_Meme() {
  // Show the modal
  let modal_add_memes_search_click = document.getElementById(
    'search-add-memes-modal-click-top-id'
  );
  modal_add_memes_search_click.style.display = 'block';
  // Get the button that opens the modal
  let meme_modal_close_btn = document.getElementById(
    'modal-search-add-memes-close-exit-view-button-id'
  );
  // When the user clicks on the button, close the modal
  meme_modal_close_btn.onclick = function () {
    const search_res_children = document.getElementById(
      'modal-search-add-memes-images-results-grid-div-area-id'
    ).children;
    const search_meme_res_children = document.getElementById(
      'modal-search-add-memes-meme-images-results-grid-div-area-id'
    ).children;
    const children_tmp = [...search_res_children, ...search_meme_res_children];
    GENERAL_HELPER_FNS.Pause_Media_From_Modals(children_tmp);
  ...
  document.getElementById(
    'modal-search-add-memes-tag-textarea-entry-id'
  ).value = '';
  document.getElementById(
    'modal-search-add-memes-tag-textarea-memes-entry-id'
  ).value = '';
  document.getElementById(
    'modal-search-add-memes-emotion-label-value-textarea-entry-id'
  ).value = '';
  document.getElementById(
    'modal-search-add-memes-emotion-meme-label-value-textarea-entry-id'
  ).value = '';
  document.getElementById(
    'modal-search-add-memes-emotion-value-range-entry-id'
  ).value = '0';
  document.getElementById(
    'modal-search-add-memes-emotion-label-value-display-container-div-id'
  ).value = '';
  document.getElementById(
    'modal-search-add-memes-emotion-meme-value-range-entry-id'
  ).value = '0';
  document.getElementById(
    'modal-search-add-memes-emotion-label-value-display-container-div-id'
  ).innerHTML = '';
  document.getElementById(
    'modal-search-add-memes-emotion-meme-label-value-display-container-div-id'
  ).innerHTML = '';
  document.getElementById(
    'modal-search-add-memes-images-results-grid-div-area-id'
  ).innerHTML = '';
  ...
  document.getElementById('modal-search-add-memes-reset-button-id').onclick =
    function () {
      document.getElementById(
        'modal-search-add-memes-tag-textarea-entry-id'
      ).value = '';
  ...
  document.getElementById(
    'modal-search-add-memes-emotion-entry-button-id'
  ).onclick = function () {
    let entered_emotion_label = document.getElementById(
      'modal-search-add-memes-emotion-label-value-textarea-entry-id'
    ).value;
    ...
    if (entered_emotion_label != '') {
      meme_tagging_search_obj['emotions'][entered_emotion_label] =
        emotion_search_entry_value;
      let image_emotions_div_id = document.getElementById(
        'modal-search-add-memes-emotion-label-value-display-container-div-id'
      );
      image_emotions_div_id.innerHTML = '';
      //Populate for the emotions of the images
      Object.keys(meme_tagging_search_obj['emotions']).forEach(
        (emotion_key) => {
          image_emotions_div_id.innerHTML += `
      ...
      Object.keys(meme_tagging_search_obj['emotions']).forEach(
        (emotion_key) => {
          document.getElementById(
            `modal-search-add-memes-emotion-remove-button-id-${emotion_key}`
          ).onclick = function () {
            let search_emotion_search_span_html_obj = document.getElementById(
              `modal-search-add-memes-emotion-label-value-span-id-${emotion_key}`
            );
            search_emotion_search_span_html_obj.remove();
  ...
  document.getElementById(
    'modal-search-add-memes-emotion-meme-entry-button-id'
  ).onclick = function () {
    let entered_emotion_label = document.getElementById(
      'modal-search-add-memes-emotion-meme-label-value-textarea-entry-id'
    ).value;
    ...
    if (entered_emotion_label != '') {
      meme_tagging_search_obj['meme_emotions'][entered_emotion_label] =
        emotion_search_entry_value;
      let image_memes_emotions_div_id = document.getElementById(
        'modal-search-add-memes-emotion-meme-label-value-display-container-div-id'
      );
      image_memes_emotions_div_id.innerHTML = '';
      //Populate for the emotions of the memes of the images
      Object.keys(meme_tagging_search_obj['meme_emotions']).forEach(
        (emotion_key) => {
          image_memes_emotions_div_id.innerHTML += `
      ...
      Object.keys(meme_tagging_search_obj['meme_emotions']).forEach(
        (emotion_key) => {
          document
            .getElementById(
              `modal-search-add-memes-emotion-meme-remove-button-id-${emotion_key}`
            )
            .addEventListener('click', function () {
              let search_meme_emotion_search_span_html_obj =
                document.getElementById(
                  `modal-search-add-memes-emotion-meme-label-value-span-id-${emotion_key}`
                );
  ...
  document.getElementById(
    'modal-search-add-memes-images-results-select-images-order-button-id'
  ).onclick = async function () {
    ...
    for (let ii = 0; ii < meme_search_results.length; ii++) {
      if (
        memes_current.includes(meme_search_results[ii]) == false &&
        current_image_annotation.fileName != meme_search_results[ii]
      ) {
        //exclude memes already present
        let meme_boolean_tmp1 = document.getElementById(
          `add-memes-images-toggle-id-${meme_search_results[ii]}`
        ).checked;
    ...
    for (let ii = 0; ii < meme_search_meme_results.length; ii++) {
      if (
        memes_current.includes(meme_search_meme_results[ii]) == false &&
        current_image_annotation.fileName != meme_search_meme_results[ii]
      ) {
        //exclude memes already present
        let meme_boolean_tmp2 = document.getElementById(
          `add-memes-meme-toggle-id-${meme_search_meme_results[ii]}`
        ).checked;
    ...
    await Update_Tagging_MEME_Connections(
      current_image_annotation.fileName,
      JSON.parse(JSON.stringify(memes_current)),
      JSON.parse(JSON.stringify(meme_switch_booleans))
    );
    meme_switch_booleans.push(...current_image_annotation.taggingMemeChoices);
    current_image_annotation.taggingMemeChoices = [
      ...new Set(meme_switch_booleans),
    ]; //add a 'unique' set of memes as the 'new Set' has unique contents
    await Update_Tagging_Annotation_DB(current_image_annotation);
    Load_State_Of_Image_IDB();
    ...
    modal_add_memes_search_click = document.getElementById(
      'search-add-memes-modal-click-top-id'
    );
    modal_add_memes_search_click.style.display = 'none';
  };
  //user presses the main search button for the add memes search
  document.getElementById('modal-search-add-memes-main-button-id').onclick =
    function () {
  ...
  for (let file_key of meme_search_results) {
    if (
      memes_current.includes(file_key) == false &&
      current_image_annotation.fileName != file_key
    ) {
      //exclude memes already present
      ...
      document.getElementById(
        ...
        if (
          document.getElementById(`add-memes-images-toggle-id-${file_key}`)
            .checked == true
        ) {
          document.getElementById(
            `add-memes-images-toggle-id-${file_key}`
          ).checked = false;
        } else {
          document.getElementById(
            `add-memes-images-toggle-id-${file_key}`
  ...
  let search_meme_images_memes_results_output = document.getElementById(
    'modal-search-add-memes-meme-images-results-grid-div-area-id'
  );
  ...
  for (let file_key of meme_search_meme_results) {
    if (
      ...
      document.getElementById(
        ...
        if (
          document.getElementById(`add-memes-meme-toggle-id-${file_key}`)
            .checked == true
        ) {
          document.getElementById(
            `add-memes-meme-toggle-id-${file_key}`
          ).checked = false;
        } else {
          document.getElementById(
            `add-memes-meme-toggle-id-${file_key}`
...
async function Modal_Meme_Search_Btn() {
  ...
  let tagging_meme_db_iterator = await Tagging_MEME_Image_DB_Iterator();
  meme_search_meme_results =
    await SEARCH_MODULE.Meme_Addition_Image_Meme_Search_DB(
      meme_tagging_search_obj,
      tagging_meme_db_iterator,
      Get_Tagging_Annotation_From_DB,
      MAX_COUNT_SEARCH_RESULTS
...
let save_modal_center_tagging = document.getElementById(
  'right-click-modal-tagging-center'
);
...
let meme_modal_image_div = document.getElementById(
  'modal-meme-clicked-image-gridbox-id'
);
let save_modal_meme_tagging = document.getElementById(
  'right-click-modal-tagging-meme'
);
    ...
    if (save_meme_tagging.style.display == 'block') {
      if (ev.target.id == 'save-file-tagging-meme') {
        ...
        if (results.canceled == false) {
          const output_name = results.filePath;
          FS.copyFileSync(
            PATH.join(TAGA_DATA_DIRECTORY, recent_meme_thumbnail_context),
            output_name,
            FS.constants.COPYFILE_EXCL
          );
          alert('saved file to download');
